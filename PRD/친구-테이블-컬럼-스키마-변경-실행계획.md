...
- [ ] DB의 friendships 테이블에서 addressee_id 컬럼이 receiver_id로 변경됨 (DESCRIBE friendships 확인)
- [ ] POST /api/friend-requests 엔드포인트가 정상 동작함 (curl 테스트)
- [ ] API 응답에서 receiver_id 필드가 올바르게 반환됨
