...
- [ ] `/user/account` 접속 시 회원 정보(이메일, 닉네임, 생년월일)가 정상 표시된다
- [ ] 데이터 로딩 중 스피너가 표시된다
- [ ] 조회 실패 시 에러 메시지가 표시된다
- [ ] 닉네임/생년월일 수정 후 "수정" 버튼 클릭 시 성공 알림이 표시된다
- [ ] 수정 성공 후 "회원 정보" 조회 영역이 자동으로 갱신된다 (Network 탭에서 GET 재요청 확인)
- [ ] DevTools → Network 탭에서 5분 내 재방문 시 GET 요청이 발생하지 않는다 (캐시 재사용)
- [ ] 수정 버튼이 요청 중 비활성화된다
