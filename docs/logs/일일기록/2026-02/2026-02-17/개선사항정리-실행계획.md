# 개선 사항 정리 실행계획 (docs/improvement-notes.md)

## 요구사항 요약

**요구사항**: 프로젝트 전반의 폴더 구조, API 설계, 프론트엔드 코드 품질 측면에서 미완성·비일관 항목을 조사하여 `docs/improvement-notes.md`에 정리한다.

**목적**: 현재 기술 부채를 명시적으로 기록하여, 이후 리팩토링·기능 개선 시 무엇을 해야 하는지 빠르게 파악할 수 있게 한다.

---

## 현재상태 분석

- `docs/improvement-notes.md` 파일 없음 — 신규 생성 필요
- 폴더 구조: `nginx.conf` 루트와 `nginx/` 디렉토리에 중복 존재, `html/` 레거시 디렉토리, 프론트엔드 `.env.example` 없음
- API 설계: `friends` 라우터만 `/api/friend-requests`(버전 없음), 인증 의존성 미사용, `print()` 디버그 문, `docker-compose.yml`에 API 토큰 하드코딩
- 프론트엔드: `services/` 폴더 비어있음, 8개 페이지에서 axios 직접 호출, 라우트 가드 없음, 환경변수 미사용

---

## 구현 방법

마크다운 형식으로 `docs/improvement-notes.md`를 신규 생성한다. 코드 변경 없이 현재 코드베이스를 분석한 결과를 문서로 정리한다. 각 항목은 문제 설명, 해당 파일 경로, 권장 조치 순서로 작성한다.
...
- 각 개선 항목은 파일 경로·문제·권장 조치 3단 구조로 작성한다.
- docker-compose 민감 정보 노출은 보안 이슈이므로 폴더 구조 섹션에 포함한다.

### 2. API 설계 개선 사항 섹션 작성
...
- 버전 불일치와 인증 누락은 운영 영향도가 높으므로 앞쪽에 배치한다.
- 각 항목에 구체적인 파일 경로와 줄 번호를 명시하여 수정 위치를 즉시 파악할 수 있게 한다.
...
- 라우트 가드는 보안과 직결되므로 첫 번째 항목으로 배치한다.
- 3.4는 3.1 해결로 연동 해소된다는 점을 명시한다.

### 4. docs/improvement-notes.md 파일 생성
...
- 1~3단계에서 작성한 내용을 하나의 파일로 합쳐 `docs/improvement-notes.md`로 저장한다.
- 파일 상단에 작성 기준일을 명시한다.
...
> 기준일: 2026-02-16 / PyPI·npm registry 실측값
...
- 2026-02-16 기준으로도 `0.20.0`이 최신이며 1.0 정식 릴리스 없음
...
- 실측 버전 데이터를 기반으로 작성한다 (추정값 사용 금지).
- 업데이트 우선순위와 주의사항을 함께 기술하여 단순 버전 나열 이상의 실용 가이드가 되도록 한다.
- `matter-js`처럼 최신이지만 pre-1.0인 경우 별도 언급한다.

### 6. 구버전 API 사용 현황 섹션 작성
...
- 각 항목에 실제 파일 경로·줄 번호·Before/After 코드 예시를 포함한다.
- 심각도(높음/중간/낮음) 기준: 향후 라이브러리 업그레이드 시 런타임 오류 가능성 여부.
- SQLAlchemy `query()` API는 현재 동작하지만 2.0 스타일 마이그레이션이 강하게 권장되므로 심각도 높음으로 분류한다.
...
| 파일 경로 | 작업 유형 | 변경 내용 |
|-----------|-----------|-----------|
| `docs/improvement-notes.md` | 생성 | 폴더 구조·API·프론트엔드·의존성 버전 개선 사항 정리 |

---

## 완료 체크리스트

- [ ] `docs/improvement-notes.md` 파일이 존재하고 열람 가능하다
- [ ] 폴더 구조 개선 항목에 `nginx.conf` 중복, 프론트엔드 `.env.example` 부재, `html/` 레거시, docker-compose 민감 정보 항목이 포함되어 있다
- [ ] API 개선 항목에 버전 불일치(`/api/friend-requests`), 인증 미적용, 디버그 print 문, 디버그 엔드포인트 노출, 상태 코드 매직 넘버, `sys.path` 하드코딩 항목이 포함되어 있다
- [ ] 프론트엔드 개선 항목에 라우트 가드 부재, API 서비스 레이어 부재, 환경변수 미사용, axios 호출 패턴 불일치 항목이 포함되어 있다
- [ ] 각 항목에 구체적인 파일 경로가 명시되어 있다
- [ ] 의존성 버전 현황 항목에 백엔드·프론트엔드 버전 비교표가 포함되어 있다
- [ ] `pytest 8→9` major 업그레이드, `fastapi` minor 다단계 차이, `matter-js` pre-1.0 고착 상태가 명시되어 있다
- [ ] 버전 수치는 실측값(PyPI·npm registry 조회)을 기반으로 한다
- [ ] 구버전 API 사용 현황 항목에 SQLAlchemy `query()` API(34곳), Pydantic `class Config` 이너 클래스(3파일), `metadata.create_all()` 프로덕션 호출, MUI Grid/size prop 혼용이 포함되어 있다
- [ ] 각 구버전 API 항목에 Before/After 코드 예시가 포함되어 있다
