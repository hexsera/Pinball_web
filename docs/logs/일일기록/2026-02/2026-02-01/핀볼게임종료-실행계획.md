...
### PHASE 2
- [o] 게임 오버 시 브라우저 개발자 도구 Console에 "Score submitted successfully" 메시지 확인
- [o] 로그인하지 않은 상태에서는 "User not logged in" 메시지 확인
- [o] DB에서 monthly_scores 테이블에 점수가 저장되었는지 확인 (`docker exec mysql-server mysql -u hexsera -phexpoint hexdb -e "SELECT * FROM monthly_scores;"`)
