...
### PHASE 1 완료 조건
- [o] `stageConfigs.js` 파일이 생성되고 STAGE_CONFIGS가 export된다
- [o] `Pinball.jsx`에 STAGE_CONFIGS가 정상적으로 import된다
- [o] 스테이지 상태(`stage`, `stageRef`, `stageBodiesRef`)가 추가된다

### PHASE 2 완료 조건
- [o] `loadStageMap()` 함수가 정상적으로 동작한다
- [o] 게임 시작 시 스테이지 1의 장애물/범퍼/목표물이 표시된다
- [o] 하드코딩된 `obstacle1`, `obstacle2`, `bumper`, `target`이 제거된다

### PHASE 3 완료 조건
- [o] 'n' 키를 누르면 스테이지 2로 전환된다
- [o] 스테이지 전환 시 생명이 3개로 초기화된다
- [o] 스테이지 전환 시 공이 초기 위치로 리셋된다
- [o] 스테이지 2의 맵 구성(장애물/범퍼/목표물)이 스테이지 1과 다르다
- [o] 스테이지 2에서 'n' 키를 눌러도 다음 스테이지가 없으면 콘솔에 메시지만 출력된다

### 전체 통합 테스트
- [o] 스테이지 2에서 생명이 모두 소진되면 게임 오버가 된다
- [o] 공이 죽음구역에 떨어졌을 때 기존과 동일하게 생명 차감 및 리스폰이 작동한다
