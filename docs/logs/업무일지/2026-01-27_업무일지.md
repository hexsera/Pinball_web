...
## 기본 정보
- 날짜: 2026-01-27 (월) 18:16~19:10
- 작성자: hexsera

## 진행 목표
핀볼 게임에 공이 닿으면 사라지는 죽음구역을 추가하여 사용자의 도전의식을 고취하고 게임에 긴장감을 더한다.

## 진행 내용

| 작업 | 실행시간 | 수정파일 |
|------|----------|----------|
| 핀볼 죽음 구역 생성 | | react/main/src/Pinball.jsx |

### 1. react/main/src/Pinball.jsx 수정

**죽음구역(deathZone) Bodies 생성 (87-95번 줄):**
```javascript
...
```
- 위치: x=350(중앙), y=1220(바닥 아래)
- `isSensor: true`로 물리적 충돌 없이 감지만 수행
- 어두운 빨간색으로 시각화 (디버깅용)

**World.add()에 deathZone 추가 (166번 줄):**
```javascript
World.add(engine.world, [
  ground, leftWall, rightWall, upWall,
  deathZone,  // ← 추가
  ball, obstacle1, obstacle2, bumper,
  leftFlipper, rightFlipper,
  leftFlipperConstraint, rightFlipperConstraint
]);
```

**collisionStart 이벤트에 죽음구역 감지 로직 추가 (205-210번 줄):**
```javascript
// 죽음구역 충돌 감지
...
Matter.js의 센서(isSensor) 기능을 활용한 죽음구역 구현:
- 센서는 충돌 이벤트는 발생시키지만 물리적 충돌(튕김, 막힘)은 발생하지 않음
- 공이 바닥 아래로 떨어지면 죽음구역과 충돌 감지
- `World.remove()`로 공을 게임 세계에서 제거하여 화면에서 사라지게 함
- label 속성으로 충돌 대상 식별 (`label: 'deathZone'`)

## 결과
- 상태: 완료
- 공이 플리퍼 사이를 빠져나가면 바닥 아래 죽음구역에서 감지되어 사라짐
- 콘솔에 'Ball entered death zone!' 메시지 출력 확인
- 게임오버 로직은 구현하지 않음 (PRD 요구사항대로)

## 다음 작업
- 공 재생성 기능 구현 (게임 계속 플레이 가능하도록)
- 죽음구역 시각적 효과 개선 (투명도 조정 또는 숨김)
- 게임오버 UI 구현 (최종 점수 표시, 재시작 버튼)