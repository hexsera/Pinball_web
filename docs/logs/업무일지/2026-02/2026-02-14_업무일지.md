...
## 기본 정보
- 날짜: 2026-02-14 (토) 19:50~20:10
- 작성자: hexsera

## 진행 목표
핀볼 플런저 영역을 개선한다. 플런저 위에 공이 안착할 발판벽을 추가하고, 발사 영역을 빨간색으로 디버그 표시하며, 플런저를 스프링 코일 모양으로 시각화한다.

## 진행 내용

| 작업 | 수정 파일 | 비고 |
|------|----------|------|
| 플런저 개선 PRD 읽고 실행계획 작성 | `PRD/핀볼-플런저-개선-실행계획.md` | 신규 생성 |
| 발판벽(plungerShelf) Body 생성 및 World 추가 | `Pinball/Pinball.jsx` | SHELF_Y 상수 추가 |
| 공 초기 위치 및 복귀 위치 발판 위로 수정 | `Pinball/Pinball.jsx` | 죽음 후 복귀, handleRestart 모두 수정 |
| ballInLane 조건을 발판 기준으로 수정 | `Pinball/Pinball.jsx` | plungerReleaseRef, handleKeyUp 2곳 |
| afterRender에 발사 영역 빨간 박스 드로잉 추가 | `Pinball/Pinball.jsx` | LAUNCH_ZONE 좌표 정의 |
| afterRender에 스프링 코일 드로잉 추가 | `Pinball/Pinball.jsx` | 플런저 위치 기반 압축 표현 |
| 발사 영역 가로세로 20씩 축소 | `Pinball/Pinball.jsx` | w: 85→65, h: 70→50 |
| ballInLane을 LAUNCH_ZONE과 동일 범위로 통일 | `Pinball/Pinball.jsx` | x: 630~695, y: SHELF_Y-25~SHELF_Y |
| 플런저 REST_Y를 발판 바로 아래로 상향 | `Pinball/Pinball.jsx` | 1050→1020 |


## 주요 변경 사항

### 1. Pinball.jsx 수정 — 발판벽 및 SHELF_Y 상수 추가
...
**특징:**
- `SHELF_Y = 1010`을 기준으로 공 위치, ballInLane, 발사 영역 등 모든 좌표를 상대 계산
- `PLUNGER_REST_Y` 1050→1020으로 올려 플런저가 발판 바로 아래에 위치
- `plungerShelf`는 너비 80px 수평 바디로 레인 내부에 배치

### 2. Pinball.jsx 수정 — 공 위치 및 ballInLane 조건 변경

**변경 전:**
```javascript
const ball = Bodies.circle(662, 1020, 15, { ... });
Body.setPosition(ball, { x: 662, y: 1020 }); // 복귀
ball.position.x > 640 && ball.position.x < 685 &&
ball.position.y > 900 && ball.position.y < 1080  // ballInLane
```

**변경 후:**
```javascript
const ball = Bodies.circle(662, SHELF_Y - 20, 15, { ... });
Body.setPosition(ball, { x: 662, y: 990 }); // 복귀
ball.position.x > 630 && ball.position.x < 695 &&
ball.position.y > SHELF_Y - 25 && ball.position.y < SHELF_Y  // ballInLane
```

**특징:**
- 공 시작·복귀 위치를 발판 위(y=990)로 변경, 죽음 후 복귀·handleRestart 모두 동일
- `ballInLane` 범위를 LAUNCH_ZONE 디버그 사각형과 정확히 일치시켜 시각과 로직 일관성 확보

### 3. Pinball.jsx 수정 — afterRender 발사 영역 + 스프링 코일 드로잉

**변경 전:**
```javascript
// afterRender에 범퍼/타겟 렌더링만 존재
```
...
// 스프링 코일 드로잉
const segH = (1100 - py) / 12; // 플런저 y에 따라 코일 간격 변화
for (let i = 0; i < 12; i++) {
  const dirX = (i % 2 === 0) ? 12 : -12;
...
**특징:**
- 발사 영역은 반투명 빨간 사각형으로 매 프레임 표시 (디버그용)
- 스프링 코일은 플런저 `position.y` 기준으로 `segH` 계산 → 당길수록 간격 좁아져 압축 표현
- 코일 색상 `#b0b0b0`, 그림자 `#888888`로 금속 느낌 연출

**플런저 영역 구조:**
```
y=985 ┌───────────────────┐ ← LAUNCH_ZONE 상단 (SHELF_Y-25)
      │  발사 영역 (빨강) │
y=1010├───────────────────┤ ← SHELF_Y / plungerShelf (발판벽)
      │                   │
y=1020│   [플런저 바디]   │ ← PLUNGER_REST_Y
      │   ↕ 스프링 코일   │
y=1100└───────────────────┘ ← 캔버스 바닥
```
