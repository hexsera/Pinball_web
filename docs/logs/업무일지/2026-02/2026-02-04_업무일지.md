...
## 기본 정보
- 날짜: 2026-02-04 (화) 17:35~18:00
- 작성자: hexsera

## 진행 목표
`/api/v1/users` 엔드포인트의 CRUD 작업에서 API Key 인증 요구사항을 제거하여 프론트엔드에서 API Key 없이 사용자 정보를 관리할 수 있도록 한다.

## 진행 내용

| 작업 | 수정 파일 | 비고 |
|------|----------|------|
| api-마스터키-제거-실행계획.md 작성 | docs/prd/ | 실행계획 문서 작성 |
| POST /api/v1/users 수정 | backend/main.py | api_key 파라미터 제거 |
| GET /api/v1/users 수정 | backend/main.py | api_key 파라미터 제거 |
| GET /api/v1/users/{user_id} 수정 | backend/main.py | api_key 파라미터 제거 |
| PUT /api/v1/users/{user_id} 수정 | backend/main.py | api_key 파라미터 제거 |
| DELETE /api/v1/users/{user_id} 수정 | backend/main.py | api_key 파라미터 제거 |
| FastAPI 서버 재시작 및 테스트 | - | API Key 없이 정상 동작 확인 |


## 주요 변경 사항

### 1. backend/main.py 수정 - POST /api/v1/users
...
- docstring에서 "API Key 필요" 문구 제거
- 기능 로직은 변경 없음

### 2. backend/main.py 수정 - GET /api/v1/users
...
### 3. backend/main.py 수정 - GET /api/v1/users/{user_id}
...
**특징:**
- `api_key` 파라미터 제거
- docstring에서 인증 정보 제거

### 4. backend/main.py 수정 - PUT /api/v1/users/{user_id}
...
### 5. backend/main.py 수정 - DELETE /api/v1/users/{user_id}
...
FastAPI Endpoint (/api/v1/users)
     │
     ├─→ verify_api_key() 검증
     │   └─→ "hexsera-secret-api-key-2026" 일치 확인
     │
     ▼
비즈니스 로직 실행
...
### 인증이 필요 없는 엔드포인트 목록 (현재)
```
인증 불필요:
├── POST /api/v1/register (회원가입)
├── POST /api/v1/login (로그인)
├── POST /api/v1/scores (점수 기록)
├── POST /api/v1/monthly-scores (월간 점수)
└── /api/v1/users/* (사용자 CRUD) ← 이번 작업으로 추가
    ├── POST /api/v1/users
    ├── GET /api/v1/users
...
## 다음 작업
- 프론트엔드 UserInfo.jsx에서 X-API-Key 헤더 제거
