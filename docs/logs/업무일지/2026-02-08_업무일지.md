...
## 기본 정보
- 날짜: 2026-02-08 (일) 19:20~20:20
- 작성자: hexsera

## 진행 목표
FastAPI 백엔드의 monolithic 구조(1011줄 main.py)를 모듈화된 구조로 리팩토링하여 유지보수성을 향상시킨다.

## 진행 내용

| 작업 | 수정 파일 | 비고 |
|------|----------|------|
| PHASE 3: DB 레이어 분리 | app/db/session.py, app/db/base.py, app/api/deps.py | DB 연결 설정 분리 |
| PHASE 4: 스키마 분리 | app/schemas/*.py (6개 파일, 31개 스키마) | Pydantic 스키마 도메인별 분리 |
| PHASE 5: 라우터 분리 | app/api/v1/*.py (7개 라우터, 23개 엔드포인트) | API 엔드포인트 도메인별 분리 |
| 구조 검증 실패 | - | main.py 리팩토링 시도 후 롤백 |

## 주요 변경 사항

### 완료된 작업
1. **DB 레이어 분리**: `app/db/session.py` (SessionLocal, engine), `app/api/deps.py` (get_db)
2. **스키마 분리**: 31개 스키마를 6개 파일로 분리 (user, score, monthly_score, friendship, game_visit, high_score)
3. **라우터 분리**: 23개 엔드포인트를 7개 라우터로 분리 (auth, users, friends, scores, monthly_scores, high_scores, game_visits)

### 발생한 문제점
1. **main.py 라우터 등록 시 trailing slash 문제**: FastAPI 라우터 경로 `/`와 prefix 조합 시 `/api/friend-requests/`가 되어 404 발생
2. **테스트 DB 데이터 격리 실패**: PostgreSQL 트랜잭션 롤백이 제대로 작동하지 않아 이전 테스트 데이터가 남음
3. **conftest.py 수정 중 문제 악화**: 테이블 초기화 로직 추가 시도 중 파일 구조가 복잡해짐
4. **Base.metadata.create_all() 미작동**: 테스트 DB(hexdb_test)에 테이블이 생성되지 않는 권한 문제 의심
5. **복구 결정**: 리팩토링으로 인한 부작용이 커져 "백엔드 스키마, 엔드포인트 분리" 커밋으로 롤백
...
## 배운내용
대규모 리팩토링은 단계별 검증이 필수적이다. 특히 테스트 인프라(conftest.py)가 불안정한 상태에서 구조 변경을 시도하면 문제 원인 파악이 어려워진다.

## 다음 작업
- 테스트 DB 격리 문제 먼저 해결 후 구조 개선 재시도
- 또는 현재 구조 유지하고 신규 기능 개발에 집중
