...
## 기본 정보
- 날짜: 2026-02-10 (화) 5:10~6:10
- 작성자: hexsera

## 진행 목표
1. 핀볼 범퍼를 일반적인 핀볼 기계처럼 시각적으로 꾸미기
2. 모든 범퍼가 공유 스타일 객체를 참조하도록 구조 개선
3. Canvas 2D API 네온 글로우 효과로 범퍼 그래픽 고품질화
...
| 작업 | 수정 파일 | 비고 |
|------|----------|------|
| BUMPER_OPTIONS 공유 상수 정의 | `frontend/src/stageConfigs.js` | 기존 인라인 객체 중복 제거 |
| 범퍼 배치 삼각형으로 재설계 | `frontend/src/stageConfigs.js` | Stage 1: 3개, Stage 2: 2개 |
| 범퍼 render를 투명으로 변경 | `frontend/src/stageConfigs.js` | afterRender 커스텀 렌더로 대체 |
| afterRender 네온 글로우 렌더링 추가 | `frontend/src/Pinball.jsx` | Canvas 2D API로 직접 그리기 |
| BUMPER_RADIUS import 추가 | `frontend/src/Pinball.jsx` | stageConfigs에서 임포트 |
| cleanup에 afterRender 해제 추가 | `frontend/src/Pinball.jsx` | 메모리 누수 방지 |

---

## 주요 변경 사항

### 1. `frontend/src/stageConfigs.js` 수정
...
**특징:**
- `BUMPER_OPTIONS` 한 곳만 수정하면 모든 스테이지의 모든 범퍼에 동시 반영
- `opacity: 0`으로 Matter.js 기본 그리기를 숨기고 afterRender에서 커스텀 렌더 전담
- Stage 1 범퍼를 삼각형 배치(3개)로 재설계하여 공이 범퍼 사이에서 튕기는 핀볼 패턴 구현
...
Events.on(render, 'afterRender', () => {
  ...
  bumperBodies.forEach((bumper) => {
    ...
    bodyGrad.addColorStop(0.5, '#cc1133');
...
Events.off(engine, 'beforeUpdate');
Events.off(engine, 'collisionStart');
Events.off(render, 'afterRender');  // 추가
```
...
│  레이어 순서 (아래 → 위)                  │
│                                          │
│  1. 외곽 글로우 1: r+12, shadowBlur 30   │  ← 가장 넓게 번지는 빨간 빛
│  2. 외곽 글로우 2: r+4,  shadowBlur 20   │  ← 중간 글로우 링
│  3. 범퍼 몸체:    r,     RadialGradient  │  ← 중앙 밝고 외곽 어두운 입체
│  4. 내부 테두리:  r-2,   strokeStyle     │  ← 선명한 경계선
│  5. 상단 반사광:  r*0.55, RadialGradient │  ← 유리/메탈 광택
└──────────────────────────────────────────┘
```

**특징:**
- Matter.js `render` 옵션으로는 단색만 지원 → Canvas 2D API `afterRender` 이벤트로 직접 그리기
- `ctx.save()` / `ctx.restore()`로 범퍼마다 Canvas 상태를 격리하여 글로우가 다른 오브젝트에 영향 없음
- `stageBodiesRef.current`에서 `label === 'bumper'`로 필터링하여 스테이지 전환 시 자동 대응
...
│      Matter.js Render        │
│  Bodies.circle 생성 시       │
│  render.fillStyle: '#e74c3c' │  ← 단색 원만 표시
└──────────────────────────────┘
...
│      Matter.js Render        │
│  Bodies.circle 생성 시       │
│  render.opacity: 0           │  ← Matter.js 기본 그리기 숨김
└──────────────┬───────────────┘
               │ afterRender 이벤트
               ▼
┌──────────────────────────────┐
