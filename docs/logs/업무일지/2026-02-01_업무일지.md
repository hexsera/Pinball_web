...
## 진행 목표
핀볼 게임에 점수증가 mechanic을 추가한다. 공이 target 오브젝트에 닿으면 점수에 300을 추가하고, 점수 컴포넌트를 상단 바에서 게임 영역 내부로 이동하며 흰색 반투명 스타일로 변경한다.

## 진행 내용

| 작업 | 수정 파일 | 비고 |
|------|----------|------|
| 실행계획 작성 | docs/prd/핀볼 점수증가-실행계획.md | PRD를 읽고 구현 단계별 실행계획 작성 |
| target 충돌 시 점수 +300 로직 추가 | react/main/src/Pinball.jsx | collisionStart 이벤트 내 기존 target 충돌 블록에 setScore 추가 |
| 상단 바에서 SCORE Typography 제거 | react/main/src/Pinball.jsx | 검은색 상단 바에 LIVES만 유지 |
| 게임 영역 내 반투명 흰색 SCORE 컴포넌트 추가 | react/main/src/Pinball.jsx | 배경이미지 Box 내부에 position: absolute로 배치 |
...
**특징:**
- `setScore(prev => prev + 300)` 형태로 이전 값 기반 갱신하여 동시성 문제 방지
- 기존 target 충돌 감지 블록에 한 줄만 추가
...
**특징:**
- SCORE Typography와 LIVES의 marginTop 제거 (LIVES가 유일한 항목이 되었으므로)
- 점수 표시는 게임 영역 내부로 이동

---

#### 게임 영역 내 반투명 흰색 SCORE 컴포넌트 추가
...
- `left: 50%` + `translateX(-50%)`로 수평 중앙 정렬
...
| 변경 항목 | 변경 전 | 변경 후 | 의도 |
|-----------|---------|---------|------|
| score 초기값 | `useState(3000)` | `useState(0)` | 실제 점수 증가로 전환하여 초기값 0으로 변경 |
| livesRef 초기값 | `useRef(3)` | `useRef(2)` | 목숨 수 조정 |
...
## 다음 작업
- 다시시작 버튼 기능 구현 (overlayState를 null로 리셋하고 공/목숨/점수 초기화)
...
## 기본 정보
- 날짜: 2026-02-01 (일) 19:20~20:20
- 작성자: hexsera

## 진행 목표
핀볼 게임 오버 시 API에 항상 0점이 전송되는 버그를 수정한다. `useEffect []` 클로저에 캡처된 `score` 초기값 문제를 `useRef`로 해결하여 실제 획득 점수가 정확히 전송되도록 한다.

## 진행 내용

| 작업 | 수정 파일 | 비고 |
|------|----------|------|
| 버그 원인 분석 | react/main/src/Pinball.jsx | `useEffect []` 클로저에서 `score`가 초기값 0으로 고정됨을 확인 |
| 실행계획 작성 | PRD/pinball-score-api-전송-버그 수정-실행계획.md | 근본원인, useRef 해결 방안, 구현 단계 작성 |
| scoreRef 생성 | react/main/src/Pinball.jsx | `useRef(0)`으로 score의 최신 값을 클로저 밖에서 접근할 수 있도록 저장 |
| score 동기화 useEffect 추가 | react/main/src/Pinball.jsx | score 상태가 변경될 때마다 scoreRef.current에 복사 |
| submitScore()에서 scoreRef.current 사용 | react/main/src/Pinball.jsx | API 전송 및 디버그 로그에서 `score` → `scoreRef.current`로 변경 |
...
**특징:**
- `useRef`로 생성된 ref 객체는 컴포넌트 수명 전체에서 동일한 객체를 유지하므로 클로저에 캡처되지 않음
- 기존 `livesRef`와 동일한 패턴으로 구현 (이미 확립된 관행)

---

#### score 동기화 useEffect 추가
...
#### submitScore()에서 scoreRef.current 사용
...
**특징:**
- API 전송 시 클로저에 고정된 `score` 대신 항상 최신 값인 `scoreRef.current`를 사용
...
```
useState(0)으로 생성된 score 상태 변수
        │
        ▼
useEffect(() => { ... }, [])  ← 마운트 시 단 한 번만 실행
        │
        ├── collisionStart 핸들러 정의  ← score = 0 캡처
        │       │
        │       └── submitScore() 호출  ← score를 항상 0으로 전송
        │
        └── score 갱신: setScore(prev => prev + 300)
                │
                ▼
        UI 리렌더링 시 최신 score 참조  ← 표시는 정상
```

**핵심:** `setScore()`는 함수형 업데이트(`prev => prev + 300`)를 사용하여 UI는 최신 값으로 갱신되지만, `collisionStart` 핸들러 안의 `score` 참조는 클로저 생성 시점의 초기값 `0`에 고정된다.

## 결과
- 상태: 완료

## 배운내용
useEffect 내부에서 useState 를 사용하면 클로저 문제가 생긴다고 한다.
useRef 를 사용해야한다.
