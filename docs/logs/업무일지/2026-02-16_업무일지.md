...
## 기본 정보
- 날짜: 2026-02-16 (월) 20:00~21:00
- 작성자: hexsera

## 진행 목표

미사용 상태인 `scores` 테이블과 관련 API, 스키마, 모델 코드를 전부 제거하여 코드베이스를 정리한다.

## 진행 내용

| 작업 | 수정 파일 | 비고 |
|------|----------|------|
| 삭제 실행계획 작성 | `PRD/scores-테이블-API-삭제-실행계획.md` | 코드베이스 분석 후 삭제 대상 및 순서 정의 |
| Score 모델 삭제 | `backend/models.py` | `Score` 클래스 블록 제거 |
| scores API 파일 삭제 | `backend/app/api/v1/scores.py` | 파일 전체 삭제 |
| v1 __init__.py scores 참조 제거 | `backend/app/api/v1/__init__.py` | import 및 `__all__` 항목 제거 |
| main.py scores 라우터 등록 제거 | `backend/main.py` | import 및 `include_router` 제거 |
| score 스키마 파일 삭제 | `backend/app/schemas/score.py` | 파일 전체 삭제 |
| schemas __init__.py score 참조 제거 | `backend/app/schemas/__init__.py` | import 및 `__all__` 항목 제거 |
| base.py Score import 제거 | `backend/app/db/base.py` | `Score` import/export 제거 |
| conftest.py scores TRUNCATE 제거 | `backend/tests/conftest.py` | `TABLES_TO_TRUNCATE`에서 `"scores"` 제거 |
| Alembic 마이그레이션 생성 및 적용 | `20e256d6d379_drop_scores_table.py` 생성 | `scores` 테이블 DB에서 DROP |
| pytest 전체 테스트 통과 확인 | - | 11/11 PASSED |
...
**특징:**
- ORM 모델에서 `Score` 클래스를 제거하여 Alembic이 `scores` 테이블을 미등록 상태로 감지
- 이후 `alembic revision --autogenerate` 실행 시 `drop_table` 마이그레이션 자동 생성

---

### 2. `backend/app/db/base.py` 수정
...
**특징:**
- `Base.metadata`에 등록되는 모델 목록에서 `Score` 제거
- Alembic의 `env.py`가 이 파일을 import하므로 여기서 제거해야 마이그레이션이 정상 동작
...
**특징:**
- `scores` 모듈 import 제거 및 라우터 등록 줄 삭제
- `POST /api/v1/scores` 엔드포인트가 완전히 제거됨

---

### 4. Alembic 마이그레이션 (`20e256d6d379_drop_scores_table.py`) 생성

**autogenerate로 자동 감지된 변경사항:**
```python
def upgrade() -> None:
...
**특징:**
- `alembic revision --autogenerate -m "drop_scores_table"` 실행으로 자동 생성
- 인덱스 4개를 먼저 삭제한 뒤 테이블 DROP (PostgreSQL 표준 순서)
- `alembic upgrade head`로 실제 DB에 반영 완료
...
| 파일 경로 | 비고 |
|-----------|------|
| `backend/app/api/v1/scores.py` | `POST /api/v1/scores` 엔드포인트 |
| `backend/app/schemas/score.py` | `ScoreCreateRequest`, `ScoreResponse`, `ScoreListResponse` 스키마 |
