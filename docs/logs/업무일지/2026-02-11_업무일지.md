# 업무일지

## 기본 정보
- 날짜: 2026-02-11 (수) 11:30~12:00 12:30~12:40
- 작성자: hexsera

## 진행 목표
AdminUserMain.jsx의 DataGrid에 회원 수정 기능을 위한 연필 아이콘 버튼을 추가하고, 버튼 클릭 시 Dialog 팝업을 띄우는 기능 구현 (TDD 방식)

## 진행 내용

| 작업 | 수정 파일 | 비고 |
|------|----------|------|
| RED: 실패 테스트 작성 (연필 버튼 렌더링) | `src/test/AdminUserMain.test.jsx` 생성 | DataGrid mock 사용 |
| RED: 실패 테스트 작성 (Dialog 팝업) | `src/test/AdminUserMain.test.jsx` | 버튼 클릭 → Dialog open |
| Vitest CSS 에러 해결 | `vite.config.js`, `src/test/setup.js` | DataGrid mock으로 우회, 임시 코드 청소 |
| GREEN: 연필 아이콘 버튼 + Dialog 구현 | `src/pages/admin/AdminUserMain.jsx` | 테스트 2개 통과 |
| 연필 아이콘 위치 변경 (오른쪽 끝) | `src/pages/admin/AdminUserMain.jsx` | actions 컬럼을 맨 뒤로 이동 |


## 주요 변경 사항

### 1. `src/test/AdminUserMain.test.jsx` 생성

**추가:**
```jsx
// DataGrid CSS 문제 우회: @mui/x-data-grid 전체 mock
vi.mock('@mui/x-data-grid', () => ({
  DataGrid: ({ rows, columns }) => (
    <table data-testid="data-grid">
      <tbody>
        {rows.map((row) => (
          <tr key={row.id}>
            {columns.map((col) => (
              <td key={col.field}>
                {col.renderCell
                  ? col.renderCell({ row })
                  : row[col.field]}
              </td>
            ))}
          </tr>
        ))}
      </tbody>
    </table>
  ),
}));

describe('AdminUserMain - 연필 아이콘 버튼', () => {
  it('각 레코드마다 연필 아이콘 버튼이 렌더링된다', async () => { ... });
  it('연필 아이콘 버튼 클릭 시 Dialog가 열린다', async () => { ... });
});
```

**특징:**
- `@mui/x-data-grid`가 내부적으로 `.css` 파일을 import하여 Vitest에서 에러 발생 → DataGrid 전체를 mock으로 교체하여 해결
- mock DataGrid는 `renderCell`을 지원하여 연필 버튼 렌더링 테스트 가능
- `axios`도 mock 처리하여 API 의존성 제거

### 2. `src/pages/admin/AdminUserMain.jsx` 수정

**변경 전:**
```jsx
const columns = [
  //{ field: 'id', headerName: 'ID', width: 70 },
  { field: 'email', headerName: '이메일', width: 200 },
  { field: 'nickname', headerName: '닉네임', width: 130 },
  { field: 'birth_date', headerName: '생년월일', width: 130 },
  { field: 'role', headerName: '역할', width: 100 },
];
```

**변경 후:**
```jsx
const columns = [
  { field: 'email', headerName: '이메일', width: 200 },
  { field: 'nickname', headerName: '닉네임', width: 130 },
  { field: 'birth_date', headerName: '생년월일', width: 130 },
  { field: 'role', headerName: '역할', width: 100 },
  {
    field: 'actions',
    headerName: '',
    width: 60,
    sortable: false,
    renderCell: (params) => (
      <IconButton
        aria-label="수정"
        size="small"
        onClick={() => handleEditClick(params.row)}
      >
        <EditIcon fontSize="small" />
      </IconButton>
    ),
  },
];
```

**특징:**
- `actions` 컬럼을 맨 오른쪽 끝에 배치
- 각 행마다 `EditIcon` 아이콘 버튼 렌더링
- 버튼 클릭 시 해당 row 데이터를 `selectedUser` 상태에 저장

**Dialog 추가:**
```jsx
// 상태 추가
const [editDialogOpen, setEditDialogOpen] = useState(false);
const [selectedUser, setSelectedUser] = useState(null);

// Dialog JSX
<Dialog open={editDialogOpen} onClose={handleDialogClose}>
  <DialogTitle>회원 수정</DialogTitle>
  <DialogContent>
    {selectedUser && (
      <Typography>
        {selectedUser.email} ({selectedUser.nickname})
      </Typography>
    )}
  </DialogContent>
  <DialogActions>
    <Button onClick={handleDialogClose}>닫기</Button>
  </DialogActions>
</Dialog>
```

### 3. `vite.config.js` 수정

**변경 전:**
```js
export default defineConfig({
  plugins: [react()],
  server: { ... },
});
```

**변경 후:**
```js
export default defineConfig({
  plugins: [react()],
  server: { ... },
  test: {
    environment: 'jsdom',
    setupFiles: ['./src/test/setup.js'],
  },
});
```

**특징:**
- Vitest 설정을 vite.config.js에 통합
- `jsdom` 환경으로 브라우저 DOM 시뮬레이션

## 결과
- 상태: 완료


## 배운 내용


## 다음 작업
- AdminUserMain.jsx Dialog에 API 연동 (GET로 폼 자동 채움, PUT으로 저장)

---

# 업무일지 2

## 기본 정보
- 날짜: 2026-02-11 (수) 12:50~13:10
- 작성자: hexsera

## 진행 목표
AdminUserMain.jsx의 Dialog 안에 유저 수정 폼 구현 (닉네임, 생년월일, 비밀번호, 역할 필드 + 저장/취소 버튼) - TDD 방식, API 연결 없음

## 진행 내용

| 작업 | 수정 파일 | 비고 |
|------|----------|------|
| RED: 닉네임 입력 필드 테스트 작성 | `src/test/AdminUserMain.test.jsx` | getByLabelText(/닉네임/i) 실패 확인 |
| GREEN: 닉네임 TextField 구현 | `src/pages/admin/AdminUserMain.jsx` | TextField label="닉네임" 추가 |
| RED: 생년월일/비밀번호/역할 필드 테스트 작성 | `src/test/AdminUserMain.test.jsx` | 3개 테스트 실패 확인 |
| GREEN: 생년월일/비밀번호/역할 필드 구현 | `src/pages/admin/AdminUserMain.jsx` | TextField 2개 + FormControl/Select 추가 |
| RED: 저장/취소 버튼 테스트 작성 | `src/test/AdminUserMain.test.jsx` | 저장 렌더링 + 취소 클릭 시 Dialog 닫힘 |
| GREEN: 저장/취소 버튼 구현 | `src/pages/admin/AdminUserMain.jsx` | 닫기 → 취소 변경, 저장 버튼 추가 |

## 주요 변경 사항

### 1. `src/test/AdminUserMain.test.jsx` 수정

**추가 (수정 폼 describe 블록):**
```jsx
describe('AdminUserMain - 수정 폼', () => {
  beforeEach(() => {
    axios.get.mockResolvedValue({ data: mockUsers });
  });

  async function openEditDialog() {
    const user = userEvent.setup();
    render(<AdminUserMain />);
    await waitFor(() => {
      expect(screen.getAllByRole('button', { name: /수정/i })).toHaveLength(mockUsers.length);
    });
    const editButtons = screen.getAllByRole('button', { name: /수정/i });
    await user.click(editButtons[0]);
    return user;
  }

  it('Dialog 안에 닉네임 입력 필드가 렌더링된다', async () => { ... });
  it('Dialog 안에 생년월일 입력 필드가 렌더링된다', async () => { ... });
  it('Dialog 안에 비밀번호 입력 필드가 렌더링된다', async () => { ... });
  it('Dialog 안에 역할 선택 필드가 렌더링된다', async () => { ... });
  it('Dialog 안에 저장 버튼이 렌더링된다', async () => { ... });
  it('취소 버튼 클릭 시 Dialog가 닫힌다', async () => { ... });
});
```

**특징:**
- `openEditDialog()` 헬퍼 함수로 Dialog 열기 과정 공유
- `취소 버튼 클릭 시 Dialog가 닫힌다` 테스트에서 MUI 애니메이션 대응을 위해 `waitFor` 사용

### 2. `src/pages/admin/AdminUserMain.jsx` 수정

**변경 전 (Dialog 내용):**
```jsx
import { ..., Button } from '@mui/material';

<Dialog open={editDialogOpen} onClose={handleDialogClose}>
  <DialogTitle>회원 수정</DialogTitle>
  <DialogContent>
    {selectedUser && (
      <Typography>
        {selectedUser.email} ({selectedUser.nickname})
      </Typography>
    )}
  </DialogContent>
  <DialogActions>
    <Button onClick={handleDialogClose}>닫기</Button>
  </DialogActions>
</Dialog>
```

**변경 후 (Dialog 내용):**
```jsx
import { ..., Button, TextField, Select, InputLabel, FormControl, MenuItem } from '@mui/material';

<Dialog open={editDialogOpen} onClose={handleDialogClose}>
  <DialogTitle>회원 수정</DialogTitle>
  <DialogContent>
    <TextField label="닉네임" fullWidth margin="dense" />
    <TextField
      label="생년월일"
      type="date"
      fullWidth
      margin="dense"
      InputLabelProps={{ shrink: true }}
    />
    <TextField label="비밀번호" type="password" fullWidth margin="dense" />
    <FormControl fullWidth margin="dense">
      <InputLabel id="role-label">역할</InputLabel>
      <Select labelId="role-label" label="역할">
        <MenuItem value="user">user</MenuItem>
        <MenuItem value="admin">admin</MenuItem>
      </Select>
    </FormControl>
  </DialogContent>
  <DialogActions>
    <Button onClick={handleDialogClose}>취소</Button>
    <Button variant="contained">저장</Button>
  </DialogActions>
</Dialog>
```

**특징:**
- 닉네임: 일반 텍스트 입력 (`TextField`)
- 생년월일: 날짜 선택 입력 (`type="date"`, `InputLabelProps={{ shrink: true }}`로 label 겹침 방지)
- 비밀번호: 비밀번호 마스킹 입력 (`type="password"`)
- 역할: `FormControl` + `Select`로 드롭다운 (user / admin 옵션)
- 기존 "닫기" 버튼 → "취소"로 변경, "저장" 버튼 신규 추가 (API 미연결)

## TDD 사이클 흐름

```
RED (닉네임 필드 테스트) → GREEN (TextField 추가)
        ↓
RED (생년월일/비밀번호/역할 테스트 3개) → GREEN (TextField 2개 + Select 추가)
        ↓
RED (저장/취소 버튼 테스트 2개) → GREEN (버튼 교체/추가)
        ↓
최종: 8개 테스트 모두 통과
```

## 결과
- 상태: 완료


## 배운 내용

## 다음 작업


---

# 업무일지 3

## 기본 정보
- 날짜: 2026-02-11 (수) 13:10~14:00
- 작성자: hexsera

## 진행 목표
AdminUserMain.jsx의 Dialog에 API를 연동한다.
1. 수정 버튼 클릭 시 GET `/api/v1/users/{user_id}`를 호출하여 닉네임, 생년월일, 역할을 폼에 자동으로 채운다. (비밀번호는 채우지 않음)
2. 저장 버튼 클릭 시 PUT `/api/v1/users/{user_id}`를 호출하여 수정된 회원 정보를 저장한다.

## 진행 내용

| 작업 | 수정 파일 | 비고 |
|------|----------|------|
| RED: 폼 자동 채움 테스트 작성 (닉네임) | `src/test/AdminUserMain.test.jsx` | `nicknameInput.value` 가 비어있어 실패 확인 |
| GREEN: editForm state + handleEditClick에 폼 초기화 구현 | `src/pages/admin/AdminUserMain.jsx` | 행 데이터로 닉네임/생년월일/역할 채움 |
| RED: 폼 자동 채움 테스트 추가 (생년월일) | `src/test/AdminUserMain.test.jsx` | 실패 확인 |
| GREEN: 생년월일 TextField value 바인딩 | `src/pages/admin/AdminUserMain.jsx` | value + onChange 연결 |
| RED: 폼 자동 채움 테스트 추가 (역할) | `src/test/AdminUserMain.test.jsx` | MUI Select value가 undefined → 실패 확인 |
| GREEN: Select value 바인딩 | `src/pages/admin/AdminUserMain.jsx` | value + onChange 연결 |
| RED: 저장 버튼 PUT API 호출 테스트 작성 | `src/test/AdminUserMain.test.jsx` | axios.put 호출 횟수 0 → 실패 확인 |
| GREEN: handleSave 구현 (PUT 호출) | `src/pages/admin/AdminUserMain.jsx` | 빈 비밀번호 제외, try/catch 처리 |
| REFACTOR: InputLabelProps → slotProps 교체 | `src/pages/admin/AdminUserMain.jsx` | MUI v5 lint 경고 제거 |
| RED: dialog 오픈 시 GET API 호출 테스트 작성 | `src/test/AdminUserMain.test.jsx` | 행 데이터 직접 사용 → GET 미호출 → 실패 확인 |
| GREEN: handleEditClick async화, GET API 호출 추가 | `src/pages/admin/AdminUserMain.jsx` | 최신 데이터 조회 후 폼 채움 |
| REFACTOR: API Key 헤더 전체 제거 | `src/pages/admin/AdminUserMain.jsx`, `src/test/AdminUserMain.test.jsx` | 3곳 제거 |

## 주요 변경 사항

### 1. `src/pages/admin/AdminUserMain.jsx` 수정

**변경 전 (`handleEditClick` - 동기, API 미호출):**
```jsx
const handleEditClick = (user) => {
  setSelectedUser(user);
  setEditDialogOpen(true);
};
```

**변경 후 (`handleEditClick` - async, GET API 호출):**
```jsx
const [editForm, setEditForm] = useState({ nickname: '', birth_date: '', password: '', role: '' });

const handleEditClick = async (user) => {
  setSelectedUser(user);
  try {
    const response = await axios.get(`/api/v1/users/${user.id}`);
    const data = response.data;
    setEditForm({ nickname: data.nickname, birth_date: data.birth_date, password: '', role: data.role });
  } catch (error) {
    console.error('회원 정보 조회 실패:', error);
    setEditForm({ nickname: user.nickname, birth_date: user.birth_date, password: '', role: user.role });
  }
  setEditDialogOpen(true);
};
```

**특징:**
- GET API 호출 실패 시 행 데이터로 fallback하여 dialog는 정상 표시
- `editForm` state로 각 필드를 controlled component로 관리
- 비밀번호 필드는 항상 빈 문자열로 초기화 (보안)

**변경 전 (저장 버튼 - API 미연결):**
```jsx
<Button variant="contained">저장</Button>
```

**변경 후 (저장 버튼 - PUT API 연동):**
```jsx
const handleSave = async () => {
  if (!selectedUser) return;
  const body = { ...editForm };
  if (!body.password) delete body.password;
  try {
    await axios.put(`/api/v1/users/${selectedUser.id}`, body);
    handleDialogClose();
  } catch (error) {
    console.error('회원 정보 수정 실패:', error);
  }
};

<Button variant="contained" onClick={handleSave}>저장</Button>
```

**특징:**
- 비밀번호가 비어있으면 PUT body에서 제외 (변경하지 않을 때 덮어쓰기 방지)
- 저장 성공 시 dialog 자동 닫힘

**변경 전 (Dialog 폼 - uncontrolled):**
```jsx
<TextField label="닉네임" fullWidth margin="dense" />
<TextField label="생년월일" type="date" fullWidth margin="dense"
  InputLabelProps={{ shrink: true }} />
<Select labelId="role-label" label="역할">...</Select>
```

**변경 후 (Dialog 폼 - controlled):**
```jsx
<TextField label="닉네임" fullWidth margin="dense"
  value={editForm.nickname}
  onChange={(e) => setEditForm({ ...editForm, nickname: e.target.value })} />
<TextField label="생년월일" type="date" fullWidth margin="dense"
  slotProps={{ inputLabel: { shrink: true } }}
  value={editForm.birth_date}
  onChange={(e) => setEditForm({ ...editForm, birth_date: e.target.value })} />
<Select labelId="role-label" label="역할"
  value={editForm.role}
  onChange={(e) => setEditForm({ ...editForm, role: e.target.value })}>
  ...
</Select>
```

### 2. `src/test/AdminUserMain.test.jsx` 수정

**추가 (수정 폼 자동 채움 describe):**
```jsx
describe('AdminUserMain - 수정 폼 자동 채움', () => {
  beforeEach(() => {
    axios.get.mockImplementation((url) => {
      if (url === '/api/v1/users') return Promise.resolve({ data: mockUsers });
      if (url === `/api/v1/users/${mockUsers[0].id}`) return Promise.resolve({ data: mockUsers[0] });
      if (url === `/api/v1/users/${mockUsers[1].id}`) return Promise.resolve({ data: mockUsers[1] });
      return Promise.resolve({ data: {} });
    });
  });

  it('Dialog를 열면 닉네임 필드에 선택한 유저의 닉네임이 채워진다', ...);
  it('Dialog를 열면 생년월일 필드에 선택한 유저의 생년월일이 채워진다', ...);
  it('Dialog를 열면 역할 필드에 선택한 유저의 역할이 채워진다', ...);
});
```

**추가 (GET API 호출 + 저장 API 연동 describe):**
```jsx
describe('AdminUserMain - dialog 오픈 시 GET API 호출', () => {
  it('수정 버튼 클릭 시 GET /api/v1/users/{user_id}를 호출한다', async () => {
    expect(axios.get).toHaveBeenCalledWith(`/api/v1/users/${mockUsers[0].id}`);
  });
});

describe('AdminUserMain - 저장 API 연동', () => {
  it('저장 버튼 클릭 시 PUT /api/v1/users/{user_id}를 호출한다', async () => {
    expect(axios.put).toHaveBeenCalledWith(
      `/api/v1/users/${mockUsers[0].id}`,
      expect.any(Object)
    );
  });
});
```

**특징:**
- `axios.get.mockImplementation`으로 URL별 응답 분기 (목록: 배열, 단건: 객체)
- MUI Select는 `.value`로 접근 불가 → dialog `textContent` 포함 여부로 역할 검증

## TDD 사이클 흐름

```
RED (닉네임 자동 채움) → GREEN (editForm state + 닉네임 value 바인딩)
        ↓
RED (생년월일 자동 채움) → GREEN (생년월일 value 바인딩)
        ↓
RED (역할 자동 채움) → GREEN (Select value 바인딩)
        ↓
RED (저장 PUT API 호출) → GREEN (handleSave 구현)
        ↓
REFACTOR (InputLabelProps → slotProps)
        ↓
RED (dialog 오픈 시 GET API 호출) → GREEN (handleEditClick async화)
        ↓
REFACTOR (API Key 헤더 전체 제거)
        ↓
최종: 13개 테스트 모두 통과
```

## dialog 데이터 흐름

```
수정 버튼 클릭
      │
      ▼
handleEditClick(user)
      │
      ▼
GET /api/v1/users/{user.id}
      │
      ├─ 성공 → setEditForm(API 응답 데이터)
      └─ 실패 → setEditForm(행 데이터 fallback)
      │
      ▼
setEditDialogOpen(true) → Dialog 표시
      │
      ▼
사용자가 폼 수정
      │
      ▼
저장 버튼 클릭 → handleSave()
      │
      ▼
PUT /api/v1/users/{selectedUser.id}
(비밀번호 비어있으면 body에서 제외)
      │
      ├─ 성공 → handleDialogClose()
      └─ 실패 → console.error
```

## 결과
- 상태: 완료

## 배운 내용

## 다음 작업


# 업무일지 4

## 기본 정보
- 날짜: 2026-02-11 (수) 14:10~15:30
- 작성자: hexsera

## 진행 목표
AdminUserMain.jsx의 DataGrid에 삭제 버튼을 추가한다. (TDD 방식)
1. 수정 버튼 오른쪽에 삭제 버튼 렌더링
2. 삭제 버튼 클릭 시 "정말로 삭제 합니까?" 확인 Dialog 표시
3. 확인 후 DELETE `/api/v1/users/{user_id}` 호출 및 DataGrid 새로고침

## 진행 내용

| 작업 | 수정 파일 | 비고 |
|------|----------|------|
| RED: 삭제 버튼 렌더링 테스트 작성 | `src/test/AdminUserMain.test.jsx` | 각 행마다 삭제 버튼 2개 기대 → 실패 확인 |
| GREEN: DeleteIcon 버튼 추가 | `src/pages/admin/AdminUserMain.jsx` | actions 컬럼에 삭제 버튼 추가 |
| RED: 삭제 확인 Dialog 테스트 작성 | `src/test/AdminUserMain.test.jsx` | "정말로 삭제 합니까?" 텍스트 기대 → 실패 확인 |
| GREEN: 삭제 확인 Dialog 구현 | `src/pages/admin/AdminUserMain.jsx` | deleteDialogOpen 상태, handleDeleteClick, Dialog JSX 추가 |
| RED: DELETE API 호출 테스트 작성 | `src/test/AdminUserMain.test.jsx` | axios.delete 호출 기대 → 실패 확인 |
| GREEN: handleDelete 구현 | `src/pages/admin/AdminUserMain.jsx` | DELETE API + fetchUsers() 호출 |
| 테스트 수정 | `src/test/AdminUserMain.test.jsx` | PUT 3번째 인자(headers) 추가, 새로고침 검증 방식 변경 |

**총 약 1시간**

## 주요 변경 사항

### 1. `src/pages/admin/AdminUserMain.jsx` 수정

**변경 전 (actions 컬럼 - 수정 버튼만):**
```jsx
import EditIcon from '@mui/icons-material/Edit';

{
  field: 'actions',
  width: 60,
  renderCell: (params) => (
    <IconButton aria-label="수정" size="small" onClick={() => handleEditClick(params.row)}>
      <EditIcon fontSize="small" />
    </IconButton>
  ),
}
```

**변경 후 (actions 컬럼 - 수정 + 삭제 버튼):**
```jsx
import EditIcon from '@mui/icons-material/Edit';
import DeleteIcon from '@mui/icons-material/Delete';

{
  field: 'actions',
  width: 120,
  renderCell: (params) => (
    <>
      <IconButton aria-label="수정" size="small" onClick={() => handleEditClick(params.row)}>
        <EditIcon fontSize="small" />
      </IconButton>
      <IconButton aria-label="삭제" size="small" onClick={() => handleDeleteClick(params.row)}>
        <DeleteIcon fontSize="small" />
      </IconButton>
    </>
  ),
}
```

**추가된 상태 및 핸들러:**
```jsx
const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);

const handleDeleteClick = (user) => {
  setSelectedUser(user);
  setDeleteDialogOpen(true);
};

const handleDeleteDialogClose = () => {
  setDeleteDialogOpen(false);
  setSelectedUser(null);
};

const handleDelete = async () => {
  if (!selectedUser) return;
  try {
    await axios.delete(`/api/v1/users/${selectedUser.id}`, {
      headers: { 'X-API-Key': 'hexsera-secret-api-key-2026' }
    });
    handleDeleteDialogClose();
    await fetchUsers();
  } catch (error) {
    console.error('회원 삭제 실패:', error);
  }
};
```

**추가된 삭제 확인 Dialog:**
```jsx
<Dialog open={deleteDialogOpen} onClose={handleDeleteDialogClose}>
  <DialogTitle>회원 삭제</DialogTitle>
  <DialogContent>
    <Typography>정말로 삭제 합니까?</Typography>
  </DialogContent>
  <DialogActions>
    <Button onClick={handleDeleteDialogClose}>취소</Button>
    <Button variant="contained" color="error" onClick={handleDelete}>삭제</Button>
  </DialogActions>
</Dialog>
```

**fetchUsers를 useEffect 밖으로 추출:**
```jsx
// 변경 전: useEffect 내부에 직접 정의
useEffect(() => {
  const fetchUsers = async () => { ... };
  fetchUsers();
}, []);

// 변경 후: 독립 함수로 추출 (삭제 후 재호출 가능)
const fetchUsers = async () => { ... };

useEffect(() => {
  fetchUsers();
}, []);
```

### 2. `src/test/AdminUserMain.test.jsx` 수정

**추가 (삭제 버튼 describe):**
```jsx
describe('AdminUserMain - 삭제 버튼', () => {
  it('각 레코드마다 삭제 버튼이 렌더링된다', async () => { ... });
  it('삭제 버튼 클릭 시 "정말로 삭제 합니까?" 확인 Dialog가 열린다', async () => { ... });
});

describe('AdminUserMain - 삭제 API 연동', () => {
  it('삭제 확인 클릭 시 DELETE /api/v1/users/{user_id}를 호출한다', async () => {
    expect(axios.delete).toHaveBeenCalledWith(
      `/api/v1/users/${mockUsers[0].id}`,
      expect.objectContaining({ headers: expect.any(Object) })
    );
  });
  it('삭제 확인 후 DataGrid가 새로고침된다', async () => {
    const callCountBefore = axios.get.mock.calls.length;
    // 삭제 후 axios.get 호출 횟수가 증가했는지 확인
    expect(axios.get.mock.calls.length).toBeGreaterThan(callCountBefore);
  });
});
```

**axios mock에 delete 추가:**
```jsx
// 변경 전
vi.mock('axios', () => ({
  default: { get: vi.fn(), put: vi.fn() },
}));

// 변경 후
vi.mock('axios', () => ({
  default: { get: vi.fn(), put: vi.fn(), delete: vi.fn() },
}));
```

**특징:**
- 새로고침 검증을 `toHaveBeenCalledTimes(2)` 대신 `callCountBefore`와 비교하는 방식으로 변경 (describe 간 호출 횟수 누적 문제 회피)
- PUT 테스트에 3번째 인자 `expect.objectContaining({ headers: ... })` 추가 (API Key 헤더 포함 검증)

## TDD 사이클 흐름

```
RED (삭제 버튼 렌더링) → GREEN (DeleteIcon 버튼 추가)
        ↓
RED (삭제 확인 Dialog) → GREEN (deleteDialogOpen 상태 + Dialog JSX)
        ↓
RED (DELETE API 호출) → GREEN (handleDelete 구현)
        ↓
최종: 17개 테스트 모두 통과
```

## 삭제 흐름

```
삭제 버튼 클릭
      │
      ▼
handleDeleteClick(user)
      │
      ▼
setDeleteDialogOpen(true) → "정말로 삭제 합니까?" Dialog 표시
      │
      ├─ 취소 클릭 → handleDeleteDialogClose()
      └─ 삭제 클릭 → handleDelete()
                          │
                          ▼
              DELETE /api/v1/users/{user_id}
              (X-API-Key 헤더 포함)
                          │
                          ├─ 성공 → handleDeleteDialogClose() → fetchUsers()
                          └─ 실패 → console.error
```

## 발견된 백엔드 이슈

삭제 시 다음 오류 발생:
```
psycopg2.errors.ForeignKeyViolation: update or delete on table "users" violates
foreign key constraint "monthly_scores_user_id_fkey" on table "monthly_scores"
```

**원인**: `monthly_scores` 테이블이 `users.id`를 외래키로 참조하고 있어, 유저 삭제 전 관련 점수 레코드를 먼저 삭제해야 함

**해결 방안 (미적용)**:
- 방법 1: `delete_user` 엔드포인트에서 `monthly_scores` 먼저 삭제
- 방법 2: FK에 `ondelete="CASCADE"` 옵션 추가 후 Alembic 마이그레이션

## 결과
- 상태: 프론트엔드 완료 / 백엔드 FK 제약 이슈 미해결

## 배운 내용

## 다음 작업
- `monthly_scores` FK 제약 해결 (CASCADE 또는 수동 삭제)
- 삭제 실패 시 사용자에게 에러 메시지 표시

# 업무일지

## 기본 정보
- 날짜: 2026-02-11 (수) 16:30~17:50
- 작성자: hexsera

## 진행 목표
홈(랜딩 페이지) 신규 생성 및 라우팅 재구성.
기존 `/` 경로에 연결된 Dashboard를 `/dashboard`로 이동하고, `/`에 랜딩 페이지를 배치한다.

## 진행 내용

| 작업 | 수정 파일 | 비고 |
|------|----------|------|
| HomePage 컴포넌트 생성 | `frontend/src/pages/HomePage/HomePage.jsx` | 신규 생성 |
| 배럴 파일 생성 | `frontend/src/pages/HomePage/index.js` | 신규 생성 |
| 라우트 수정 | `frontend/src/App.jsx` | `/` → HomePage, `/dashboard` → Dashboard 추가 |
| 로그인 후 이동 경로 수정 | `frontend/src/pages/Login/Login.jsx` | `navigate('/')` → `navigate('/dashboard')` |
| Hero 섹션 가운데 정렬 | `frontend/src/pages/HomePage/HomePage.jsx` | Container maxWidth="lg" 적용 |
| Grid 여백 문제 해결 | `frontend/src/pages/HomePage/HomePage.jsx` | Grid v2 방식으로 size prop 수정 |

## 주요 변경 사항

### 1. HomePage/HomePage.jsx 생성

**추가:**
```jsx
// 네비게이션 바 + Hero 섹션 (CTA 좌측, 랭킹 테이블 우측) + 푸터
// 다크 네이비(#0F172A) 배경, 인디고(#4F46E5) 포인트 컬러
// 더미 랭킹 데이터 (useState 초기값으로 하드코딩)
// Container maxWidth="lg"로 가로 가운데 정렬
```

**특징:**
- Dashboard를 수정하지 않고 새 컴포넌트로 분리
- 랭킹 데이터는 더미 데이터로 표시 (API 연동 전 단계)
- 1~3위 메달 이모지(🥇🥈🥉) 조건부 표시

---

### 2. App.jsx 수정

**변경 전:**
```jsx
<Route path="/" element={<Dashboard />} />
```

**변경 후:**
```jsx
<Route path="/" element={<HomePage />} />
<Route path="/dashboard" element={<Dashboard />} />
```

**특징:**
- 기존 Dashboard 라우트는 `/dashboard`로 이동 (코드 변경 없음)
- `/`에 신규 HomePage 연결

---

### 3. Login.jsx 수정

**변경 전:**
```jsx
navigate('/');  // 일반 사용자는 대시보드로
```

**변경 후:**
```jsx
navigate('/dashboard');  // 일반 사용자는 대시보드로
```

---

### 4. Grid 여백 문제 해결

**문제:**
랭킹 카드 오른쪽에 여백이 발생했다.

**원인:**
MUI v7의 `Grid`는 Grid v2로 `gap` 기반으로 동작한다. 기존 Grid v1 방식인 `item xs={12} md={6}` prop을 사용하면 너비 계산이 잘못되어 오른쪽 아이템이 컨테이너를 벗어난다.

**변경 전:**
```jsx
<Grid item xs={12} md={6}>
```

**변경 후:**
```jsx
<Grid size={{ xs: 12, md: 4 }}>
```

**특징:**
- MUI v7에서는 `item` prop 대신 `size` prop을 사용해야 한다.
- `item xs md` 방식은 GridLegacy에서만 유효하다.

## 결과
- 상태: 완료

## 배운내용

## 다음 작업
- 랭킹 테이블 실제 API 연동 (GET /api/v1/scores/ranking 구현 후)
- 로그인 상태일 때 네비게이션 바 버튼 변경 (로그인/회원가입 → 닉네임/로그아웃)

---

# 업무일지 5

## 기본 정보
- 날짜: 2026-02-11 (수) 17:50~18:20
- 작성자: hexsera

## 진행 목표
React Bits Aurora 컴포넌트를 설치하여 HomePage.jsx 배경에 WebGL 오로라 애니메이션 효과를 적용한다.

## 진행 내용

| 작업 | 수정 파일 | 비고 |
|------|----------|------|
| ogl 패키지 설치 | `frontend/package.json` | Aurora WebGL 렌더링 의존성 |
| Aurora 컴포넌트 생성 | `frontend/src/components/Aurora/Aurora.jsx` | React Bits 공식 소스 복사 |
| Aurora CSS 생성 | `frontend/src/components/Aurora/Aurora.css` | `.aurora-container { width/height: 100% }` |
| HomePage.jsx Aurora 배경 적용 | `frontend/src/pages/HomePage/HomePage.jsx` | position absolute로 배경 레이어 구성 |
| Aurora Box 높이 100vh 설정 | `frontend/src/pages/HomePage/HomePage.jsx` | 사용자 요청으로 height: '100vh' 추가 |

## 주요 변경 사항

### 1. `frontend/package.json` 수정

**변경 전:**
```json
"matter-js": "^0.20.0",
"react": "^19.2.3",
```

**변경 후:**
```json
"matter-js": "^0.20.0",
"ogl": "^1.0.11",
"react": "^19.2.3",
```

**특징:**
- `ogl`은 경량 WebGL 라이브러리로 Aurora 셰이더 렌더링에 사용
- React Bits Aurora 컴포넌트가 내부적으로 `ogl`의 `Renderer`, `Program`, `Mesh`, `Color`, `Triangle`을 사용

---

### 2. `frontend/src/components/Aurora/Aurora.jsx` 생성

**추가:**
```jsx
import { Renderer, Program, Mesh, Color, Triangle } from 'ogl';
import { useEffect, useRef } from 'react';
import './Aurora.css';

export default function Aurora(props) {
  const { colorStops = ['#5227FF', '#7cff67', '#5227FF'], amplitude = 1.0, blend = 0.5 } = props;
  // ...
  return <div ref={ctnDom} className="aurora-container" />;
}
```

**특징:**
- React Bits 공식 소스코드 그대로 사용 (JS-CSS 버전)
- GLSL 셰이더(VERT, FRAG)로 Simplex 노이즈 기반 오로라 애니메이션 생성
- `useEffect`로 WebGL 컨텍스트를 초기화하고, `requestAnimationFrame`으로 매 프레임 렌더링
- 컴포넌트 언마운트 시 WebGL 컨텍스트 해제 및 이벤트 리스너 정리

**props:**
| prop | 기본값 | 설명 |
|------|--------|------|
| `colorStops` | `['#5227FF', '#7cff67', '#5227FF']` | 오로라 색상 배열 (3개) |
| `amplitude` | `1.0` | 오로라 높이 진폭 |
| `blend` | `0.5` | 색상 혼합 범위 |
| `speed` | `1.0` | 애니메이션 속도 배수 |

---

### 3. `frontend/src/pages/HomePage/HomePage.jsx` 수정

**변경 전:**
```jsx
return (
  <Box sx={{ minHeight: '100vh', backgroundColor: COLORS.bg, display: 'flex', flexDirection: 'column' }}>
    {/* 네비게이션 바 */}
    ...
```

**변경 후:**
```jsx
import Aurora from '../../components/Aurora/Aurora';

return (
  <Box sx={{ position: 'relative', minHeight: '100vh', backgroundColor: COLORS.bg, display: 'flex', flexDirection: 'column' }}>
    {/* Aurora 배경 */}
    <Box sx={{ position: 'absolute', inset: 0, zIndex: 0, height: '100vh', overflow: 'hidden' }}>
      <Aurora
        colorStops={['#467ee5', '#7C3AED', '#908aff']}
        amplitude={1.2}
        speed={1.2}
        blend={0.35}
      />
    </Box>

    {/* 기존 콘텐츠 */}
    <Box sx={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      {/* 네비게이션 바, Hero 섹션, 푸터 */}
    </Box>
  </Box>
);
```

**레이어 구조:**
```
<Box (최상위, position: relative)>
  │
  ├── <Box (Aurora 배경, position: absolute, zIndex: 0, height: 100vh)>
  │       └── <Aurora /> (WebGL 캔버스)
  │
  └── <Box (콘텐츠, position: relative, zIndex: 1)>
          ├── AppBar (네비게이션 바)
          ├── Container (Hero 섹션 + 랭킹 테이블)
          └── Box footer (푸터)
```

**특징:**
- Aurora Box를 `position: absolute, inset: 0`으로 설정하여 콘텐츠 레이아웃에 영향 없이 배경에 깔림
- 기존 콘텐츠 Box에 `zIndex: 1`을 부여하여 Aurora 위에 정상 표시
- `colorStops`를 `#4F46E5`(인디고) 계열로 설정하여 기존 홈페이지 테마와 통일

## 결과
- 상태: 완료

## 배운내용

## 다음 작업


# 업무일지 6

## 기본 정보
- 날짜: 2026-02-11 (수) 19:10~19:30
- 작성자: hexsera

## 진행 목표
Dashboard.jsx에서 불필요한 UI 요소를 제거하여 핵심 기능만 남긴다.
1. Dashboard 접속 시 "게임하기" 화면이 기본으로 표시되도록 변경
2. 사이드바 메뉴에서 "메인페이지", "소식", "통계" 제거
3. 헤더 검색바 제거
4. `mainele` 변수 주석 처리 및 렌더에서 제거

## 진행 내용

| 작업 | 수정 파일 | 비고 |
|------|----------|------|
| `showPinball` 초기값 `false` → `true` 변경 | `frontend/src/pages/Dashboard/Dashboard.jsx` | 접속 시 즉시 게임 화면 표시 |
| `menuItems` 배열 축소 (6개 → 3개) | `frontend/src/pages/Dashboard/Dashboard.jsx` | 메인페이지, 소식, 통계 제거 |
| `handleMenuClick` 불필요한 분기 제거 | `frontend/src/pages/Dashboard/Dashboard.jsx` | 메인페이지, 소식, 통계 분기 삭제 |
| 헤더 `TextField` 검색바 제거 | `frontend/src/pages/Dashboard/Dashboard.jsx` | InputAdornment도 함께 제거 |
| 불필요한 import 제거 | `frontend/src/pages/Dashboard/Dashboard.jsx` | 12개 미사용 import 삭제 |
| `mainele` 변수 전체 주석 처리 | `frontend/src/pages/Dashboard/Dashboard.jsx` | 코드 보존, 렌더에서 null로 교체 |

## 주요 변경 사항

### 1. `frontend/src/pages/Dashboard/Dashboard.jsx` 수정

**변경 전 (`showPinball` 초기값):**
```jsx
const [showPinball, setShowPinball] = useState(false);
```

**변경 후:**
```jsx
const [showPinball, setShowPinball] = useState(true);
```

**특징:**
- `true`로 설정하면 Dashboard 진입 즉시 `<Pinball />` 컴포넌트가 렌더링됨

---

**변경 전 (`menuItems` - 6개):**
```jsx
const menuItems = [
  { text: '메인페이지', icon: <Dashboard />, path: '/' },
  { text: '게임하기', icon: <CalendarMonth />, path: '/calendar' },
  { text: '친구', icon: <Person />, path: '/profile' },
  { text: '소식', icon: <Description />, submenu: [...] },
  { text: '통계', icon: <BarChart />, submenu: [...] },
  { text: '계정', icon: <Lock />, submenu: [...] },PRD/홈페이지 레이아웃PRD.md
];
```

**변경 후 (`menuItems` - 3개):**
```jsx
const menuItems = [
  { text: '게임하기', icon: <CalendarMonth />, path: '/calendar' },
  { text: '친구', icon: <Person />, path: '/profile' },
  { text: '계정', icon: <Lock />, submenu: [...] },
];
```

**특징:**
- 메인페이지(index 0)가 제거되어 게임하기가 첫 번째 항목(index 0)이 됨
- `menuIndex` 초기값 0이 게임하기를 가리키게 되어 접속 시 게임하기 메뉴가 하이라이트됨

---

**변경 전 (헤더 검색바 포함):**
```jsx
<Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
  <IconButton ...><MenuIcon /></IconButton>
  <TextField
    placeholder="Type to search..."
    size="small"
    sx={{ width: 300, display: { xs: 'none', md: 'block' } }}
    InputProps={{ startAdornment: <InputAdornment ...><Search /></InputAdornment> }}
  />
</Box>
```

**변경 후 (검색바 제거):**
```jsx
<Box sx={{ display: 'flex', alignItems: 'center' }}>
  <IconButton ...><MenuIcon /></IconButton>
</Box>
```

**특징:**
- `TextField`, `InputAdornment`, `Search` 아이콘 모두 제거
- `gap: 2`도 불필요하여 함께 제거

---

**변경 전 (렌더 조건부 표현식):**
```jsx
{showPinball ? <Pinball /> : showUserInfo ? <UserInfo /> : showFriend ? <FriendPage /> : mainele}
```

**변경 후:**
```jsx
{showPinball ? <Pinball /> : showUserInfo ? <UserInfo /> : showFriend ? <FriendPage /> : null}
```

**특징:**
- `mainele` 변수는 주석 처리하여 코드를 보존하되, 렌더에서는 `null`로 교체
- `showPinball`이 항상 `true`로 시작하므로 사실상 `null` 분기에 도달하지 않음

---

**제거된 import 목록:**
```jsx
// '@mui/material'에서 제거
TextField, InputAdornment, Avatar, Badge, Menu, MenuItem

// '@mui/icons-material'에서 제거
Dashboard, Description, BarChart, Search, Notifications, Mail
```

## 사이드바 메뉴 구조 변화

```
변경 전                    변경 후
────────────────           ────────────────
메인페이지 (index 0)        게임하기 (index 0)  ← 기본 선택
게임하기   (index 1)        친구     (index 1)
친구       (index 2)        계정     (index 2)
소식       (index 3)
통계       (index 4)
계정       (index 5)
```

## 결과
- 상태: 완료

## 배운내용

## 다음 작업

---

# 업무일지 7

## 기본 정보
- 날짜: 2026-02-11 (수) 7:50~8:10
- 작성자: hexsera

## 진행 목표
HomePage.jsx의 하드코딩된 더미 랭킹 데이터를 실제 API로 교체한다.
`GET /api/v1/monthly-scores` 호출 후 상위 10위까지만 표시. (TDD 방식)

## 진행 내용

| 작업 | 수정 파일 | 비고 |
|------|----------|------|
| RED: API 호출 테스트 작성 (4개) | `src/test/HomePage.test.jsx` 생성 | Aurora WebGL mock 처리 포함 |
| RED 검증: 테스트 실패 확인 | — | 4개 중 3개 실패 (API 미구현), 1개 우연히 통과 |
| GREEN: `useEffect` + `axios.get` API 연동 구현 | `src/pages/HomePage/HomePage.jsx` | 더미 데이터 제거, slice(0, 10) 적용 |
| GREEN 검증: 4개 테스트 모두 통과 확인 | — | |
| REFACTOR: `key={index}` → `key={row.nickname}` | `src/pages/HomePage/HomePage.jsx` | 안정적인 key 값으로 교체 |
| REFACTOR 검증: 4개 테스트 모두 통과 확인 | — | |

## 주요 변경 사항

### 1. `src/test/HomePage.test.jsx` 생성

**추가:**
```jsx
vi.mock('axios');
vi.mock('../components/Aurora/Aurora', () => ({
  default: () => <div data-testid="aurora-mock" />,
}));

describe('HomePage 랭킹 API 연동', () => {
  it('페이지 접속 시 GET /api/v1/monthly-scores 를 호출한다', ...);
  it('API 응답의 닉네임이 랭킹 테이블에 표시된다', ...);
  it('최대 10위까지만 표시된다', ...);
  it('API 호출 실패 시 에러 메시지를 표시한다', ...);
});
```

**특징:**
- Aurora 컴포넌트가 `ogl`(WebGL)을 사용하여 테스트 환경에서 오류 발생 → `vi.mock`으로 더미 div로 교체
- 11개 항목의 mock 데이터에서 10위까지만 표시되는지 검증
- API 실패 시 `"랭킹을 불러올 수 없습니다."` 텍스트 표시 여부 검증

### 2. `src/pages/HomePage/HomePage.jsx` 수정

**변경 전 (하드코딩 더미 데이터):**
```jsx
import { useState } from 'react';

const [ranking] = useState([
  { rank: 1, nickname: 'alpha', score: 1234500 },
  { rank: 2, nickname: 'ninja', score: 987300 },
  { rank: 3, nickname: 'hexfan', score: 765000 },
  { rank: 4, nickname: 'king', score: 543200 },
  { rank: 5, nickname: '홍길동', score: 321000 },
]);
```

**변경 후 (API 연동):**
```jsx
import { useState, useEffect } from 'react';
import axios from 'axios';

const [ranking, setRanking] = useState([]);
const [rankingError, setRankingError] = useState(false);

useEffect(() => {
  axios.get('/api/v1/monthly-scores')
    .then((res) => {
      setRanking(res.data.scores.slice(0, 10));
    })
    .catch(() => {
      setRankingError(true);
    });
}, []);
```

**특징:**
- 더미 데이터 완전 제거, 실제 API 응답으로 교체
- `.slice(0, 10)`으로 상위 10위만 표시
- API 실패 시 `rankingError` 상태를 `true`로 설정

**변경 전 (랭킹 테이블 렌더):**
```jsx
{ranking.map((row) => (
  <TableRow key={row.rank}>
    <TableCell>
      {row.rank === 1 ? '🥇' : row.rank === 2 ? '🥈' : row.rank === 3 ? '🥉' : row.rank}
    </TableCell>
    <TableCell>{row.nickname}</TableCell>
    <TableCell align="right">{row.score.toLocaleString()}</TableCell>
  </TableRow>
))}
```

**변경 후 (에러 분기 + rank 인덱스 계산):**
```jsx
{rankingError ? (
  <Typography sx={{ color: COLORS.subText }}>랭킹을 불러올 수 없습니다.</Typography>
) : (
  <TableBody>
    {ranking.map((row, index) => {
      const rank = index + 1;
      return (
        <TableRow key={row.nickname}>
          <TableCell>
            {rank === 1 ? '🥇' : rank === 2 ? '🥈' : rank === 3 ? '🥉' : rank}
          </TableCell>
          <TableCell>{row.nickname}</TableCell>
          <TableCell align="right">{row.score.toLocaleString()}</TableCell>
        </TableRow>
      );
    })}
  </TableBody>
)}
```

**특징:**
- `row.rank` 대신 `index + 1`로 순위 계산 (API 응답에 rank 필드 없음)
- `key={row.rank}` → `key={row.nickname}`으로 변경 (더 안정적인 고유값)
- 에러 시 테이블 대신 에러 메시지 Typography 표시

## API 데이터 흐름

```
HomePage 마운트
      │
      ▼
useEffect → axios.get('/api/v1/monthly-scores')
      │
      ├─ 성공 → res.data.scores.slice(0, 10) → setRanking()
      │                    │
      │                    ▼
      │          랭킹 테이블에 닉네임/점수 표시 (최대 10개)
      │
      └─ 실패 → setRankingError(true)
                           │
                           ▼
                  "랭킹을 불러올 수 없습니다." 표시
```

## TDD 사이클 흐름

```
RED (4개 테스트 작성) → 3개 실패 확인
        ↓
GREEN (useEffect + axios.get 구현) → 4개 모두 통과
        ↓
REFACTOR (key={index} → key={row.nickname})
        ↓
최종: 4개 테스트 모두 통과
```

## 결과
- 상태: 완료

## 배운 내용

## 다음 작업
- 로그인 상태일 때 네비게이션 바 버튼 변경 (로그인/회원가입 → 닉네임/로그아웃)
- `monthly_scores` FK 제약 해결 (사용자 삭제 시 CASCADE 또는 수동 삭제)
