...
### Phase 2: 페이지 컴포넌트 이동 (Login, Register)
...
### Phase 3: 페이지 컴포넌트 이동 (Dashboard, UserInfo, Pinball)

```javascript
// src/pages/Dashboard/Dashboard.jsx
// 기존 src/Dashboard.jsx 파일 내용 이동
// import 경로만 상대경로로 수정 (Sidebar/Header 분리는 Phase 6에서 진행)
import { useAuth } from '../../contexts/AuthContext';
import Pinball from '../Pinball';
import UserInfo from '../UserInfo';
```
- Dashboard.jsx를 pages/Dashboard/ 폴더로 이동
- 기존 인라인 Sidebar/Header 로직 유지 (Phase 6에서 컴포넌트로 분리)
...
// import 경로만 수정 (userService 연동은 Phase 7에서 진행)
import { useAuth } from '../../contexts/AuthContext';
```
- UserInfo.jsx를 pages/UserInfo/ 폴더로 이동
- 기존 axios 직접 호출 유지 (Phase 7에서 userService로 교체)
...
### Phase 4: Admin 페이지 이동 및 App.jsx 업데이트
...
```
- pages/Dashboard/Dashboard.jsx에서 Sidebar 로직 추출
- props로 메뉴 클릭 핸들러 전달
- 재사용 가능한 독립 컴포넌트로 분리
...
```
- pages/Dashboard/Dashboard.jsx에서 Header 로직 추출
...
```javascript
// pages/Dashboard/Dashboard.jsx 수정
// 인라인 Sidebar/Header 로직을 분리된 컴포넌트로 교체
import { Sidebar, Header } from '../../components/layout';
```
- Dashboard.jsx에서 인라인 Sidebar/Header 코드 제거
- 분리된 컴포넌트로 교체

### Phase 7: API 서비스 레이어 생성
...
```javascript
// 각 컴포넌트에서 axios 직접 호출을 service로 교체
...
// pages/UserInfo/UserInfo.jsx
import { userService } from '../../services';
```
- Login, Register, UserInfo의 axios 직접 호출을 service 함수로 교체
- 컴포넌트에서 API 호출 로직 완전 제거

## 수정/생성할 파일 목록

| Phase | 파일 경로 | 작업 유형 | 변경 내용 |
|-------|-----------|-----------|-----------|
| 0 | `frontend/` | 이동 | `react/main/`에서 이동 (폴더명 변경) |
| 0 | `CLAUDE.md` | 수정 | react/main 경로를 frontend로 변경 |
| 1 | `src/contexts/AuthContext.jsx` | 이동 | `src/AuthContext.jsx`에서 이동 |
| 1 | `src/contexts/index.js` | 생성 | barrel export 파일 |
| 2 | `src/pages/Login/Login.jsx` | 이동 | `src/Login.jsx`에서 이동, import 경로 수정 |
| 2 | `src/pages/Login/index.js` | 생성 | barrel export 파일 |
| 2 | `src/pages/Register/Register.jsx` | 이동 | `src/Register.jsx`에서 이동, import 경로 수정 |
| 2 | `src/pages/Register/index.js` | 생성 | barrel export 파일 |
| 3 | `src/pages/Dashboard/Dashboard.jsx` | 이동 | `src/Dashboard.jsx`에서 이동, import 경로 수정 |
| 3 | `src/pages/Dashboard/index.js` | 생성 | barrel export 파일 |
| 3 | `src/pages/UserInfo/UserInfo.jsx` | 이동 | `src/UserInfo.jsx`에서 이동, import 경로 수정 |
| 3 | `src/pages/UserInfo/index.js` | 생성 | barrel export 파일 |
| 3 | `src/pages/Pinball/Pinball.jsx` | 이동 | `src/Pinball.jsx`에서 이동 |
| 3 | `src/pages/Pinball/index.js` | 생성 | barrel export 파일 |
| 4 | `src/pages/admin/*.jsx` | 이동 | `src/admin/*.jsx`에서 이동 |
| 4 | `src/pages/admin/index.js` | 생성 | barrel export 파일 |
| 4 | `src/App.jsx` | 수정 | import 경로 업데이트 |
| 5 | `vite.config.js` | 수정 | 경로 별칭 추가 (선택 사항) |
| 6 | `src/components/layout/Sidebar.jsx` | 생성 | Dashboard에서 Sidebar 로직 추출 |
| 6 | `src/components/layout/Header.jsx` | 생성 | Dashboard에서 Header 로직 추출 |
| 6 | `src/components/layout/index.js` | 생성 | barrel export 파일 |
| 6 | `src/pages/Dashboard/Dashboard.jsx` | 수정 | 인라인 Sidebar/Header를 컴포넌트로 교체 |
| 7 | `src/services/api.js` | 생성 | axios 인스턴스 중앙화 |
| 7 | `src/services/authService.js` | 생성 | 인증 API 함수 |
| 7 | `src/services/userService.js` | 생성 | 사용자 API 함수 |
| 7 | `src/services/index.js` | 생성 | barrel export 파일 |
| 7 | `src/pages/Login/Login.jsx` | 수정 | axios 직접 호출 → authService 사용 |
| 7 | `src/pages/Register/Register.jsx` | 수정 | axios 직접 호출 → authService 사용 |
| 7 | `src/pages/UserInfo/UserInfo.jsx` | 수정 | axios 직접 호출 → userService 사용 |
...
**Phase 1 체크리스트** (완료):
- [x] 디렉토리 구조 생성 (components, pages, contexts, hooks, services, utils)
- [x] AuthContext.jsx를 src/contexts/로 이동
- [x] src/contexts/index.js barrel export 생성
- [x] App.jsx에서 import 경로 수정
- [x] `npm run build` 에러 없이 통과

**Phase 2 체크리스트**:
- [ ] Login.jsx가 pages/Login/Login.jsx로 이동되었는지 확인
- [ ] Register.jsx가 pages/Register/Register.jsx로 이동되었는지 확인
- [ ] 각 페이지 index.js barrel export 생성 확인
- [ ] `npm run build` 에러 없이 통과

**Phase 3 체크리스트**:
- [ ] Dashboard.jsx가 pages/Dashboard/Dashboard.jsx로 이동되었는지 확인
- [ ] UserInfo.jsx가 pages/UserInfo/UserInfo.jsx로 이동되었는지 확인
- [ ] Pinball.jsx가 pages/Pinball/Pinball.jsx로 이동되었는지 확인
- [ ] Pinball 게임이 정상 작동하는지 확인
- [ ] `npm run build` 에러 없이 통과

**Phase 4 체크리스트**:
- [ ] admin/*.jsx가 pages/admin/으로 이동되었는지 확인
- [ ] App.jsx의 모든 import 경로가 새 구조로 업데이트되었는지 확인
- [ ] 모든 라우팅이 정상 작동하는지 확인
- [ ] `npm run build` 에러 없이 통과

**Phase 5 체크리스트**:
- [ ] vite.config.js에 경로 별칭 추가 확인
- [ ] `@contexts`, `@services` 등 별칭 import가 동작하는지 확인
- [ ] `npm run build` 에러 없이 통과

**Phase 6 체크리스트**:
- [ ] Sidebar.jsx가 components/layout/에 생성되었는지 확인
- [ ] Header.jsx가 components/layout/에 생성되었는지 확인
- [ ] Dashboard.jsx에서 인라인 Sidebar/Header가 컴포넌트로 교체되었는지 확인
- [ ] Dashboard 화면이 정상적으로 표시되는지 확인
- [ ] `npm run build` 에러 없이 통과

**Phase 7 체크리스트**:
- [ ] services/api.js, authService.js, userService.js 생성 확인
- [ ] Login, Register의 axios 호출이 authService로 교체되었는지 확인
- [ ] UserInfo의 axios 호출이 userService로 교체되었는지 확인
- [ ] 로그인/회원가입/회원정보 기능이 정상 작동하는지 확인
