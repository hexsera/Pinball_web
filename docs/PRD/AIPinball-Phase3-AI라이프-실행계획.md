...
- [ ] 게임 시작 시 상단바에 빨간 하트 3개(플레이어)와 파란 하트 3개(AI)가 표시된다
- [ ] 공이 화면 상단 aiDeathZone에 닿으면 파란 하트가 1개 줄어든다
- [ ] AI 하트가 남아있을 때 aiDeathZone 충돌 후 공이 중앙(x=350, y=550)에서 재발사된다
- [ ] AI 하트가 0개가 되면 "YOU WIN" overlay가 표시된다
- [ ] 공이 바닥 deathZone에 닿으면 빨간 하트가 줄고, 0이 되면 "GAME OVER" overlay가 표시된다
- [ ] "다시 시작" 버튼 클릭 시 플레이어·AI 하트 모두 3개로 초기화된다
- [ ] 게임 중 G키를 누르면 공이 반대 방향으로 떨어진다 (중력 반전)
- [ ] G키를 한 번 더 누르면 중력이 원래대로 돌아온다
- [ ] 브라우저 콘솔에 에러 없이 실행된다
