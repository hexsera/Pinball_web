...
- [x] 브라우저에서 게임 시작 시 공이 x=350, y=550 위치에서 출발하는지 확인
- [x] 공이 왼쪽 아래 또는 오른쪽 아래 방향 중 하나로 랜덤 발사되는지 확인
- [x] 목숨 감소 후 공 복귀 시 처음과 동일한 방향으로 재발사되는지 확인
- [x] 재시작 버튼 클릭 후 공이 중앙에서 발사되는지 확인
- [x] 화면 오른쪽(x=630~700)에 플런저/발판벽 물리 바디가 시각적으로 없는지 확인
- [x] 모바일에서 중앙 플런저 버튼이 사라지고 좌/우 버튼만 남는지 확인
- [x] 콘솔에 `plunger`, `plungerShelf` 관련 참조 오류가 없는지 확인
- [x] Space 키로 게임 시작이 정상 동작하는지 확인
